...
export default function SectionedItemList({ items }: Props) {
  const sections: ItemSection[] = [
    {
      ...
      data: items.filter(
        (item) =>
          ...
          item.weapon_type === WeaponType.Melee
      ),
    ...
    },
    {
      ...
      data: items.filter(
        (item) =>
          item.item_type === ItemType.Weapon &&
          item.weapon_type === WeaponType.Ranged
      ),
    ...
    },
    {
      ...
      data: items.filter(
        (item) =>
          item.item_type === ItemType.Weapon &&
          item.weapon_type === WeaponType.Blackpowder
      ),
  ...
  return (
    <ThemedView style={{ flex: 1, padding: 16 }}>
      <SectionedList<ItemObject>
        sections={sections}
        renderItem={({ item }: { item: ItemObject }) => (
